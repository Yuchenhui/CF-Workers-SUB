...
export default {
	async fetch(request, env) {
		...
		const guestSubscription = guestToken;
		//console.log(`${fakeUserID}\n${fakeHostName}`); // 打印fakeID

		// 简化：移除未使用的UD和expire变量
		SUBUpdateTime = env.SUBUPTIME || SUBUpdateTime;

		if (!([mytoken, fakeToken, guestSubscription].includes(token) || url.pathname == ("/" + mytoken) || url.pathname.includes("/" + mytoken + "?"))) {
			...
			if (env.KV) {
				...
				if (userAgent.includes('mozilla') && !url.search) {
					...
					return await KV(request, env, 'LINK.txt', guestSubscription);
				} else {
			...
			let aggregatedLinks = await ADD(MainData + '\n' + urls.join('\n'));
			let customNodes = "";
			let subscriptionUrls = "";
			for (let x of aggregatedLinks) {
			...
			urls = await ADD(subscriptionUrls);
			await sendMessage(`#获取订阅 ${FileName}`, request.headers.get('CF-Connecting-IP'), `UA: ${userAgentHeader}</tg-spoiler>\n域名: ${url.hostname}\n<tg-spoiler>入口: ${url.pathname + url.search}</tg-spoiler>`);

			// 默认为clash格式
			let subscriptionFormat = 'clash';

			// 根据URL参数或User-Agent检测格式
			if (url.searchParams.has('b64') || url.searchParams.has('base64')) {
				subscriptionFormat = 'base64';
			} else if (url.searchParams.has('sb') || url.searchParams.has('singbox') || userAgent.includes('sing-box') || userAgent.includes('singbox')) {
				subscriptionFormat = 'singbox';
			} else if (url.searchParams.has('surge') || userAgent.includes('surge')) {
				subscriptionFormat = 'surge';
			} else if (url.searchParams.has('quanx') || userAgent.includes('quantumult')) {
				subscriptionFormat = 'quanx';
			} else if (url.searchParams.has('loon') || userAgent.includes('loon')) {
				subscriptionFormat = 'loon';
			}

			let subConverterUrl;
			let conversionUrl = `${url.origin}/${await MD5MD5(fakeToken)}?token=${fakeToken}`;
			let req_data = MainData;

			// 默认使用clash作为User-Agent
			let appendedUserAgent = 'clash';

			// 根据订阅格式设置对应的User-Agent
			if (subscriptionFormat === 'singbox') appendedUserAgent = 'singbox';
			else if (subscriptionFormat === 'surge') appendedUserAgent = 'surge';
			else if (subscriptionFormat === 'quanx') appendedUserAgent = 'Quantumult%20X';
			else if (subscriptionFormat === 'loon') appendedUserAgent = 'Loon';
			...
			if (env.WARP) conversionUrl += "|" + (await ADD(env.WARP)).join("|");
			// 去重处理
			...
			const target = targetMap[subscriptionFormat] || 'clash';
			const extraParams = subscriptionFormat === 'quanx' ? '&udp=true' : '';

			subConverterUrl = `${subProtocol}://${subConverter}/sub?target=${target}&url=${encodeURIComponent(conversionUrl)}&insert=false&config=${encodeURIComponent(subConfig)}&emoji=true&list=false&tfo=false&scv=true&fdn=false&sort=false&new_name=true${extraParams}`;
			try {
		...
		for (const response of modifiedResponses) {
			...
			if (response.status === 'fulfilled') {
				...
				if (content.includes('proxies:')) {
					...
					conversionUrls += "|" + response.apiUrl; // Clash 配置
				} else if (content.includes('outbounds"') && content.includes('inbounds"')) {
					//console.log('Singbox订阅: ' + response.apiUrl);
					conversionUrls += "|" + response.apiUrl; // Singbox 配置
